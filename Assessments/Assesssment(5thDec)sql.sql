...
--(Use aggregate + joins)
select top 3 p.productid, p.productname, sum(od.qty * p.price) as totalsales from orderdetails od join products p on od.productid = p.productid
...
from customers c left join orders o on c.custid = o.custid group by c.city
...
join products p on od.productid = p.productid group by od.orderid having sum(od.qty * p.price)>10000
...
create view vv_lowstockproducts
...
sum(od.qty * p.price) as totalamount from orders o
join orderdetails od on o.orderid = od.orderid
...
as
begin
if (@newprice <= 0) throw 50001, 'invalid price', 1
...
exec sp_updateproductprice @productid = 106, @newprice = 500
...
as
begin
select distinct o.orderid, o.orderdate, c.custname, c.city
from orders o
...
exec sp_searchorders @city = 'delhi'
...
create trigger trg_nodelete_product
on products
instead of delete
as
begin
...
throw 50002, 'cannot delete: product used in orders', 1
...
select d.paymentid, d.amount, i.amount, getdate() 
from deleted d
...
update Payments set Amount =500 where PaymentID = 7001
